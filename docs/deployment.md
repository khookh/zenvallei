...
For a local Pages-path check before pushing, run `pnpm build:pages` followed by `pnpm test:pages`. If official raster data changed, first run `pnpm official-layers:publish` and `pnpm data:validate`. The Pages smoke test asserts the exact eight-layer inventory and activates every published dataset below `/zenvallei/`.

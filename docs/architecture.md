...
- Comparison modules are separate from the top-level layer registry. One explicit seven-pair table supports Heat with income or population; Landsat with Urban Atlas, JaarBAK, Green Map or income; and Green Map with income. Either participant discovers the same canonical comparison; the coordinator switches to that presentation and restores the initiating layer when it is removed. Prepared comparison assets are lazy and use catalogue schema 3 in local and static builds.
- The three comparisons that need an exact sealed footprint share one focused raster-composition module. It reads the same JaarBAK PMTiles as the standalone layer, keeps only official red sealed pixels and can intersect them with the five eligible Urban Atlas fabric classes. This is display composition only: statistical calculations continue to use their documented native or aligned analytical grids.

...
- Comparison modules are separate from the top-level layer registry. One explicit five-pair table supports Heat with income or population, Landsat with Urban Atlas or JaarBAK, and Green Map with Urban Atlas. Either participant discovers the same canonical comparison; the coordinator switches to that canonical presentation and restores the initiating layer when it is removed. Prepared comparison assets are lazy and use catalogue schema 3 in local and static builds.

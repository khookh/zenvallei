...
| Land-cover change tool, local only | Green Map 2021, Soil sealing 2024, Urban Atlas 2021, Radoux et al. (2025) and clear Landsat 22 June 2026 | `processing/local-layers/.../lst_scenario.py`, `scenario_land_cover.py`, `image_regression_optuna.py` | schema-5 local manifest, one optional verified 2026 booster/baseline inference cache and session-only method-keyed ΔLST rasters | latent edit ground resolved to one exclusive 1 m upper surface; Radoux 15 m Gaussian support and XGBoost radial predictors on 30 m thermal centres |

...
In Landsat comparison mode, Urban Atlas is reduced to the selected analysis families or classes beneath a stronger thermal raster. The JaarBAK comparison draws a deep-red aligned mask only for majority-sealed Landsat cells below the thermal raster, while its histogram compares sealed and unsealed assignments. Histograms use fixed 0.5 degree Celsius bins; each curve is normalised independently and is descriptive rather than causal. See [Landsat surface temperature](landsat-surface-temperature.md) for exact family membership, year pairing and limitations.
...
Three sealed-urban comparisons use Urban Atlas classes `11100`, `11210`, `11220`, `11230` and `11240`, explicitly excluding isolated structures `11300`. JaarBAK must classify the cell as majority sealed. Green Map × Landsat compares eligible 30 m pixels; Green Map × income compares eligible sector means on the 10 m density grid; Landsat × income compares eligible sector mean temperatures. The charts show ordinary unweighted OLS lines and R² as descriptive summaries only. Spatial dependence, ecological aggregation and mismatched source years prevent causal interpretation; no p-values are reported.

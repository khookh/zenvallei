...
In Landsat comparison mode, Urban Atlas is reduced to the selected analysis families or classes beneath a stronger thermal raster. The JaarBAK comparison reuses the official matched PMTiles: only the exact bright-red 1 m sealed pixels are drawn below the complete Landsat observation. Its histogram still compares 30 m sealed and unsealed analytical assignments. A second chart plots every eligible clear Landsat record against the surrounding 100 m soil-sealing density without sampling. Histograms use fixed 0.5 degree Celsius bins; all relationships are descriptive rather than causal. See [Landsat surface temperature](landsat-surface-temperature.md) for exact family membership, year pairing and limitations.
...
Three sealed-urban comparisons use Urban Atlas classes `11100`, `11210`, `11220`, `11230` and `11240`, explicitly excluding isolated structures `11300`. The Green Map comparison maps use the exact 1 m sealed footprint intersected with this Urban Atlas mask. Green Map × income weights every 10 m density value by its exact eligible sealed area; Green Map × Landsat keeps its established one-record-per-eligible-30 m-pixel graph contract while displaying exact sealed subpixels. Landsat × income compares eligible sector mean temperatures. The charts show ordinary unweighted OLS lines and R² as descriptive summaries only. Spatial dependence, ecological aggregation and mismatched source years prevent causal interpretation; no p-values are reported.

...
const permittedOfficialAsset = /(?:^|\/)data\/(?:official-layers\/(?:[a-z0-9/_-]+\.(?:bin\.gz|geojson|json|pmtiles|tif))|population\/population-density-2019\.tif)$/i;
...
const scenarioBytes = (await Promise.all(files
  .filter((file) => file.includes(`${path.sep}data${path.sep}official-layers${path.sep}land-cover-scenario${path.sep}`))
  .map(async (file) => (await fs.stat(file)).size))).reduce((sum, size) => sum + size, 0);
if (scenarioBytes > 90 * 1024 * 1024) throw new Error(`Published scenario assets exceed the 90 MiB budget (${scenarioBytes} bytes).`);
if (officialBytes > 650 * 1024 * 1024) throw new Error(`Published official layers exceed the 650 MiB budget (${officialBytes} bytes).`);
